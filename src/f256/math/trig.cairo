...
use crate::f256::types::fixed::{
    Fixed, FixedTrait, FixedAdd, FixedSub, FixedMul, FixedDiv, ONE_u256,
};

// CONSTANTS

const PI_u256: u256 = 57952155664616982739;
const HALF_PI_u256: u256 = 28976077832308491370;
...
fn acos(a: Fixed) -> Fixed {
    ...
    if (a.sign) {
        return FixedTrait::new(PI_u256, false) - asin_res;
    } else {
        return asin_res;
    }
}
...
fn asin(a: Fixed) -> Fixed {
    assert(a.mag <= ONE_u256, 'out of range');

    if (a.mag == ONE_u256) {
        return FixedTrait::new(HALF_PI_u256, a.sign);
    }
...
fn atan(a: Fixed) -> Fixed {
    ...
    if (invert) {
        res = res - FixedTrait::new(HALF_PI_u256, false);
    }
...
fn cos(a: Fixed) -> Fixed {
    return sin(FixedTrait::new(HALF_PI_u256, false) - a);
}

fn cos_fast(a: Fixed) -> Fixed {
    return sin_fast(FixedTrait::new(HALF_PI_u256, false) - a);
}

fn sin(a: Fixed) -> Fixed {
    let a1_u256 = a.mag % (2 * PI_u256);
    let (whole_rem, partial_rem, _) = u256_safe_divmod(a1_u256, u256_as_non_zero(PI_u256));
...
fn sin_fast(a: Fixed) -> Fixed {
    let a1_u256 = a.mag % (2 * PI_u256);
    let (whole_rem, mut partial_rem, _) = u256_safe_divmod(a1_u256, u256_as_non_zero(PI_u256));
    let partial_sign = whole_rem == 1;

    if partial_rem >= HALF_PI_u256 {
        partial_rem = PI_u256 - partial_rem;
    }
...
fn _sin_loop(a: Fixed, i: u256, acc: Fixed) -> Fixed {
    let div_u256 = (2 * i + 2) * (2 * i + 3);
    let term = a * a * acc / FixedTrait::new_unscaled(div_u256, false);
...
mod tests {
    ...
    use crate::f256::test::helpers::{assert_precise, assert_relative};
    use crate::f256::types::fixed::{ONE, FixedInto, FixedPartialEq, FixedPrint};

    use super::{
        FixedTrait, ONE_u256, acos, atan_fast, atan, asin, sin, HALF_PI_u256, cos, PI_u256,
    ...
    fn test_acos() {
        ...
        assert(acos(a).try_into().unwrap() == 0, 'invalid one');

        let a = FixedTrait::new(ONE_u256 / 2, false);
        assert(
            acos(a).try_into().unwrap() == 19317385211018935530, 'invalid half',
        ); // 1.0471975506263043

        let a = FixedTrait::ZERO();
        assert(acos(a).try_into().unwrap() == 28976077832308491370, 'invalid zero'); // PI / 2

        let a = FixedTrait::new(ONE_u256 / 2, true);
        assert(
            acos(a).try_into().unwrap() == 38634770453598047209, 'invalid neg half',
        ); // 2.094395102963489

        let a = FixedTrait::new(ONE_u256, true);
        assert(acos(a).try_into().unwrap() == 57952155664616982739, 'invalid neg one'); // PI
    }
    ...
    #[available_gas(100000000000)]
    fn test_acos_fail() {
        let a = FixedTrait::new(2 * ONE_u256, true);
        acos(a);
    }
    ...
    fn test_atan_fast() {
        ...
        let a = FixedTrait::ZERO();
        assert(atan_fast(a).try_into().unwrap() == 0, 'invalid zero');
    ...
    fn test_atan() {
        ...
        let a = FixedTrait::ZERO();
        assert(atan(a).try_into().unwrap() == 0, 'invalid zero');
    ...
    fn test_asin() {
        let a = FixedTrait::ONE();
        assert(asin(a).try_into().unwrap() == 28976077832308491370, 'invalid one'); // PI / 2

        let a = FixedTrait::new(ONE_u256 / 2, false);
        assert(asin(a).try_into().unwrap() == 9658692617570005102, 'invalid half');

        let a = FixedTrait::ZERO();
        assert(asin(a).try_into().unwrap() == 0, 'invalid zero');

        let a = FixedTrait::new(ONE_u256 / 2, true);
        assert(asin(a).try_into().unwrap() == -9658692617570005102, 'invalid neg half');

        let a = FixedTrait::new(ONE_u256, true);
        assert(asin(a).try_into().unwrap() == -28976077832308491370, 'invalid neg one'); // -PI / 2
    }
    ...
    fn test_cos() {
        ...
        assert(cos(a).try_into().unwrap() == 0, 'invalid half pi');
        ...
        let a = FixedTrait::new(PI_u256, false);
        assert_precise(cos(a), -18446744073709552000, 'invalid pi', Option::None(()));

        let a = FixedTrait::new(HALF_PI_u256, true);
        assert(cos(a).try_into().unwrap() == -1, 'invalid neg half pi'); // -0.000...
    ...
    fn test_cos_fast() {
        ...
        let a = FixedTrait::new(HALF_PI_u256, false);
        assert(cos_fast(a).try_into().unwrap() == 0, 'invalid half pi');
        ...
        let a = FixedTrait::new(HALF_PI_u256, true);
        assert(cos_fast(a).try_into().unwrap() == 0, 'invalid neg half pi');
    ...
    fn test_sin() {
        ...
        let a = FixedTrait::new(PI_u256, false);
        assert(sin(a).try_into().unwrap() == 0, 'invalid pi');
    ...
    fn test_sin_fast() {
        ...
        let a = FixedTrait::new(PI_u256, false);
        assert(sin_fast(a).try_into().unwrap() == 0, 'invalid pi');
    ...
    fn test_compare_sin() {
        ...
        loop {
            ...
            assert_precise(sin1, sin2.mag.try_into().unwrap(), 'invalid sin', error);
    ...
    fn test_tan() {
        let a = FixedTrait::new(HALF_PI_u256 / 2, false);
        assert(tan(a).try_into().unwrap() == ONE, 'invalid quarter pi');

        let a = FixedTrait::new(PI_u256, false);
        assert(tan(a).try_into().unwrap() == 0, 'invalid pi');
