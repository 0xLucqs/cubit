...
fn cosh(a: Fixed) -> Fixed {
    ...
    let den = FixedTrait::new_unscaled(2_u256, false);
    return num / den;
}
...
fn atanh(a: Fixed) -> Fixed {
    ...
    return ln_arg.ln() / FixedTrait::new_unscaled(2_u256, false);
}
...
mod tests {
    ...
    use crate::f256::types::fixed::{FixedInto, FixedPartialEq, ONE};

    use super::{FixedTrait, cosh, ONE_u256, sinh, tanh, acosh, asinh, atanh};
    ...
    fn test_cosh() {
        ...
        let a = FixedTrait::new(0_u256, false);
    ...
    fn test_sinh() {
        ...
        let a = FixedTrait::new(0_u256, false);
        assert(sinh(a).try_into().unwrap() == 0, 'invalid zero');
    ...
    fn test_tanh() {
        ...
        let a = FixedTrait::new(0_u256, false);
        assert(tanh(a).try_into().unwrap() == 0, 'invalid zero');
    ...
    #[available_gas(100000000000)]
    fn test_acosh() {
        let a = FixedTrait::new(69400261067392811864_u256, false); // 3.762195691016423
        assert_precise(acosh(a), 2 * ONE, 'invalid two', Option::None(()));

        let a = FixedTrait::new(28464813554960036081_u256, false); // 1.5430806347841253
        assert_precise(acosh(a), ONE, 'invalid one', Option::None(()));

        let a = FixedTrait::ONE(); // 1
        assert(acosh(a).try_into().unwrap() == 0, 'invalid zero');
    }

    #[test]
    #[available_gas(100000000000)]
    fn test_asinh() {
        let a = FixedTrait::new(66903765733337761105_u256, false); // 3.6268604077773023
        assert_precise(asinh(a), 2 * ONE, 'invalid two', Option::None(()));

        let a = FixedTrait::new(21678635653511457631_u256, false); // 1.1752011936029418
        assert_precise(asinh(a), ONE, 'invalid one', Option::None(()));

        let a = FixedTrait::new(0_u256, false);
        assert(asinh(a).try_into().unwrap() == 0, 'invalid zero');
    ...
    fn test_atanh() {
        ...
        let a = FixedTrait::new(0_u256, false);
        assert(atanh(a).try_into().unwrap() == 0, 'invalid zero');
